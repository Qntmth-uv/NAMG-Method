...
class latexTable:
    ...
    def __init__(self, name:str, scalebox:float = 0.58, colors:bool = True):
    ...
    def __headerTable__(self)->None:        
        ...
        header_file_name = f"""        \\multicolumn{{15}}{{|c|}}{{\\cellcolor{{PerlWhite}}\\textbf{{Problem Name: {self.function_name}}}}}"""+TU.TAB_END
    ...
    def __tableBody__(self, factors_matrices: list[np.ndarray])->None:
        ...
        for f in range(factors_matrices[0].shape[0]):
            ...
            if f==0:
                ...
                print(f"        \\multirow{{3}}{{*}}{{$10^{{{f}}}$}}" + TU.TAB_LINKER + self.__elementTable__(factors_matrices[0][f], deactivate_tab_enders=True) + TU.TAB_LINKER + "S" +TU.TAB_END)
                print("        " + TU.TAB_LINKER + self.__elementTable__(factors_matrices[1][f], True) + TU.TAB_LINKER + "O" +TU.TAB_END)
                print(        "" + TU.TAB_LINKER + self.__elementTable__(factors_matrices[2][f], True) + TU.TAB_LINKER + "W"+ TU.TAB_END)
                continue
            elif f==1:
                print("        \\cmidrule(lr){1-15}")            
            else: 
            ...
            print(f"        \\multirow{{3}}{{*}}{{$10^{{{f}}}$}}" + TU.TAB_LINKER + self.__elementTable__(factors_matrices[0][f]))
            print("        " + TU.TAB_LINKER + self.__elementTable__(factors_matrices[1][f]))
            print("        " + TU.TAB_LINKER + self.__elementTable__(factors_matrices[2][f]))
        return None 


    def __bottomTable__(self, caption:str = ""):
    ...
    def generateTable(self, factors_matrices: list[np.ndarray], caption:str="")->None:
